...
class KeywordSortWindow(tk.Toplevel):
    def __init__(self, manager):
        super().__init__()
        self.title('Configuration - Rules Folder: Keywords')
        self.resizable(False, False)
        # Main Window object to manipulate the attribute containing the keywords dictionary.
        ...
        self.__txt_keywords = None  # Text Entry -> Extensions that will be saved in the folder
        self.__file_json = 'config\\keywords.json'  # Relative path of json keywords file.
        self.init_widgets()
        self.read_config()
    ...
    def init_widgets(self):
        ...
        top_frame = ttk.LabelFrame(self, text='New Rule Keywords')
        ...
        self.__txt_keywords = tk.Text(middle_frame, width=30, height=10)
        self.__txt_keywords.grid(row=0, column=1, rowspan=3)
    ...
    def read_config(self):
        ...
        try:
            self.__manager.keywords_data = read(self.__file_json)
    ...
    def read_keywords(self):
        ...
        if self.__folder_selected is not None:
            ...
            chain_ext = ''
            for keyword in self.__manager.keywords_data[key]:
                chain_ext += f'{keyword}; '
            self.__txt_keywords.delete("1.0", "end")
            self.__txt_keywords.insert(tk.INSERT, chain_ext)

    def load_keywords(self):
        """Get the text of the text widget and update the value in the dictionary."""

        if self.__folder_selected is not None:
            key_text = self.__txt_keywords.get(1.0, tk.END + '-1c')
            key_text = key_text.replace(' ', '')
            key_list = key_text.split(';')
            for i in range(len(key_list)):
                if key_list[i] == '':
                    key_list.pop(i)
            self.__manager.keywords_data[self.__folder_selected] = key_list
    ...
    def load_folder_name(self):
        ...
        try:
            if self.__folder_name.get() != '':
                self.__manager.keywords_data[self.__folder_name.get()] = []
                self.read_folder_name()
    ...
    def delete(self):
        ...
        try:
            if self.__folder_selected is not None:
                ...
                if ans == 'yes':
                    del self.__manager.keywords_data[self.__folder_selected]
                    self.__txt_keywords.delete("1.0", "end")
                    self.read_folder_name()
    ...
    def write_config(self):
        ...
        try:
            write(self.__manager.keywords_data, self.__file_json)
